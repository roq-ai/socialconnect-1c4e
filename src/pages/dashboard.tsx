...
import { getNotificationSettings } from 'apiSdk/notification-settings';
import { NotificationSettingsInterface } from 'interfaces/notification-settings';
import { getContents } from 'apiSdk/contents';
import { ContentInterface } from 'interfaces/content';

import { ContentListPage as ShowCaseList } from 'pages/contents';
...
function DashboardPage() {
  ...
  const notificationSettingsStat = useSWR<PaginatedInterface<NotificationSettingsInterface>>(
    () => `/notification-settings`,
    () => getNotificationSettings({ limit: 1, offset: 0 }),
  );

  const contentStat = useSWR<PaginatedInterface<ContentInterface>>(
    () => `/contents`,
    () => getContents({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: notificationSettingsStat?.data?.totalCount,
      isLoading: notificationSettingsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Contents',
      result: contentStat?.data?.totalCount,
      isLoading: contentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
