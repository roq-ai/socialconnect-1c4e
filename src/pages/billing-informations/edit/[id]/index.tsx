...
import { getBillingInformationById, updateBillingInformationById } from 'apiSdk/billing-informations';
...
function BillingInformationEditPage() {
  ...
  const handleSubmit = async (values: BillingInformationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBillingInformationById(id, values);
      mutate(updated);
      resetForm();
