...
import { getBillingInformationById } from 'apiSdk/billing-informations';
import { BillingInformationInterface } from 'interfaces/billing-information';

function BillingInformationViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BillingInformationInterface>(
    () => (id ? `/billing-informations/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('billing_information', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/billing-informations/edit/${id}`} passHref legacyBehavior>
                    <Button
