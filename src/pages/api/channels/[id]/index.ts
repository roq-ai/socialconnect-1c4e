...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.channel
    .withAuthorization({
      roqUserId,
  ...
  async function getChannelById() {
    const data = await prisma.channel.findFirst(convertQueryToPrismaUtil(req.query, 'channel'));
    return res.status(200).json(data);
  }

  async function updateChannelById() {
