...
import { getChannels, deleteChannelById } from 'apiSdk/channels';
import { ChannelInterface } from 'interfaces/channel';

type ColumnType = ColumnDef<ChannelInterface, unknown>;
...
export function ChannelListPage(props: ChannelListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ChannelInterface>>(
    () => `/channels?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ChannelInterface) => {
    if (hasAccess('channel', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/channels/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/channels/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('channel', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/channels/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('channel', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="channel" />
        </Flex>

        {hasAccess('channel', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/channels/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
