...
import { getSocialAccountById, updateSocialAccountById } from 'apiSdk/social-accounts';
...
function SocialAccountEditPage() {
  ...
  const handleSubmit = async (values: SocialAccountInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSocialAccountById(id, values);
      mutate(updated);
      resetForm();
