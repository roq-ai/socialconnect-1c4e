...
type ColumnType = ColumnDef<SubscriptionInterface, unknown>;
...
export function SubscriptionListPage(props: SubscriptionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SubscriptionInterface>>(
    () => `/subscriptions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SubscriptionInterface) => {
    if (hasAccess('subscription', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/subscriptions/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="subscription" />
        </Flex>
